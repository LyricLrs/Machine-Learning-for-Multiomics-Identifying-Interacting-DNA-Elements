...
for i in np.arange(32):
    peak_gene_results.append(
        pd.read_csv(f'/results/SCENT_outputs/SCENT_outputs_{i+1}.csv', sep=' ')
    )
...
print("="*20+"unique_genes")
print(pd.Series(unique_genes))
...
for i in np.arange(32):
    print("="*20 + enhancer_pair_split_df[i])
    print(enhancer_pair_split_df[i].head(3))
