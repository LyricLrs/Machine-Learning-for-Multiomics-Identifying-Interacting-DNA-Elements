...
peak_gene_results.to_csv('/scratch/az1932/CDS-2024-Fall-Capstone/results/SCENT_peak_gene/significant_peak_gene_associations.csv', index = False)

# write significant genes to separate file (for GSEA)
unique_genes = peak_gene_results['gene'].unique()
pd.Series(unique_genes).to_csv('/scratch/az1932/CDS-2024-Fall-Capstone/results/SCENT_peak_gene/SCENT_gene_set.csv', index = False)
...
for i in np.arange(32):
    enhancer_pair_split_df[i].to_csv(f'/scratch/az1932/CDS-2024-Fall-Capstone/results/enhancer_pairs/enhancer_pairs_{i+1}.csv', index = False)
